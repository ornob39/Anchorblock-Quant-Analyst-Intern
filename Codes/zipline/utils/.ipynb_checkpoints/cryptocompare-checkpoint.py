...
def getSymbolFromExchange(exchange='Coinbase', conversion='USD',
    ...
    """
    base = f"https://min-api.cryptocompare.com/data/v2/pair/mapping/exchange?e={exchange}"
    ...
    coin_list = [pair['exchange_fsym'] for pair in response['Data']['current'] if pair['exchange_tsym'] == conversion]
    return coin_list

def getExchanges(apikey="5c7ed28f27882a16f8483b7aeefa0fb7f59ca7f89a9da7040e0f750db3e04713"):
...
def coinList(exchanges = ['Coinbase'], conversion='USD',
            apikey="5c7ed28f27882a16f8483b7aeefa0fb7f59ca7f89a9da7040e0f750db3e04713"):
...
def getDailyBars(start,
    ...
    df['timestamp'] = df['timestamp'].apply(lambda x: datetime.fromtimestamp(x, pytz.utc).replace(hour=0, minute=0, second=0))
    df=df.set_index('timestamp')
    return df

def coinLibrary(start,
                end,
                coin_list,
                conversion='USD',
...
def downloader(coin_dict, savedir):
    ...
    for coin in list(coin_dict.keys()):
        df = getDailyBars(start=coin_dict[coin]['start'],
