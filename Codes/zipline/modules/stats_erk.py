...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None, rebalance=7, periods=252):
    """
    ...
    if isinstance(risky_r, pd.Series): 
        risky_r = pd.DataFrame(risky_r.values, index=risky_r.index,  columns=["R"])

    if safe_r is None:
        safe_r = pd.DataFrame().reindex_like(risky_r)
        safe_r.values[:] = riskfree_rate/periods # fast way to set all values to a number
    ...
    last_balanced = -1*rebalance
    for step in range(n_steps):
        if drawdown is not None:
            peak = np.maximum(peak, account_value)
            floor_value = peak*(1-drawdown)
        cushion = (account_value - floor_value)/account_value
        # rebalance only once per x days
        if step-last_balanced==rebalance:
            risky_w = m*cushion
    ...
    # cushion_history.ffill(axis=0, inplace=True)
    # risky_w_history.ffill(axis=0, inplace=True)
    # account_history.ffill(axis=0, inplace=True)
    # floorval_history.ffill(axis=0, inplace=True)
    # peak_history.ffill(axis=0, inplace=True)
    backtest_result = {
...
def summary_stats(r, periods, riskfree_rate=0.03):
    """
